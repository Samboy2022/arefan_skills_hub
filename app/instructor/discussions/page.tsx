...
// ── Types & Helpers ────────────────────────────────────────────────────────────
type InstructorDiscussionReply = DiscussionComment["replies"][0] & {
  is_instructor?: boolean;
};

type InstructorDiscussionComment = Omit<DiscussionComment, "replies"> & {
  replies: InstructorDiscussionReply[];
};

type InstructorDiscussionThread = Omit<DiscussionThread, "comments"> & {
  is_gradable?: boolean;
  mark_obtainable?: number;
  comments?: InstructorDiscussionComment[];
};

const CURRENT_USER_ID = "instructor_1";
const CURRENT_USER_NAME = "Instructor Smith";
const CURRENT_USER_AVATAR = "https://api.dicebear.com/7.x/avataaars/svg?seed=Teacher";
...
// ── Thread Row ─────────────────────────────────────────────────────────────────
function ThreadRow({ thread, onClick }: { thread: InstructorDiscussionThread; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
function ThreadDetailView({
  ...
  thread: InstructorDiscussionThread;
  onBack: () => void;
  isGeneral: boolean;
  onEdit?: () => void;
  onDelete?: () => void;
}) {
  const [comments, setComments] = useState<InstructorDiscussionComment[]>(thread.comments ?? []);
  ...
  const handleComment = () => {
    ...
    const newComment: InstructorDiscussionComment = {
      id: `c${Date.now()}`,
      thread_id: thread.id,
      ...
      likes: 0,
      replies: [],
      is_instructor: true, // Special Instructor Flag
    };
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {isOwner && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800">Instructor (You)</span>}
                          ...
                          <div key={reply.id} className="flex items-start gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-0.5">
                                ...
                                {reply.is_instructor && (
                                   <span className="text-[9px] font-bold uppercase px-1 py-0.5 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800">Instructor</span>
                                )}
...
function CreateGeneralThreadForm({ onCancel, onSubmit }: { onCancel: () => void; onSubmit: (t: InstructorDiscussionThread) => void }) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");

  const handleCreate = () => {
    if (!title.trim() || !description.trim()) return;
    const thread: InstructorDiscussionThread = {
      id: `gen${Date.now()}`,
      course_id: null,
      forum_type: "general",
      title,
    ...
    <div className="space-y-5 max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-5 bg-card border rounded-lg p-6 shadow-sm">
        <div>
          ...
          <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Best coding practices?" />
        </div>
        <div>
...
function CreateCourseThreadForm({ onCancel, onSubmit }: { onCancel: () => void; onSubmit: (t: InstructorDiscussionThread) => void }) {
...
// ── Edit Thread Form ───────────────────────────────────────────────────────────
function EditThreadForm({ thread, onCancel, onSave }: { thread: InstructorDiscussionThread; onCancel: () => void; onSave: (title: string, description: string) => void }) {
...
function ForumList({
  ...
  threads: InstructorDiscussionThread[];
  isGeneral: boolean;
  courseId?: string;
  courseName?: string;
  onBack: () => void;
}) {
  const [allThreads, setAllThreads] = useState<InstructorDiscussionThread[]>(threads);
  const [view, setView] = useState<"list" | "thread" | "create_general" | "create_course" | "edit">("list");
  const [activeThread, setActiveThread] = useState<InstructorDiscussionThread | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  const pinned = allThreads.filter(t => t.is_pinned);
  const regular = allThreads.filter(t => !t.is_pinned);
  const filtered = (arr: InstructorDiscussionThread[]) =>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {isGeneral
              ...
              : `Course-specific discussion. You have administrative controls.`}
          </p>
        </div>
...
// ── Main Page: Instructor Dashboard ────────────────────────────────────────────
...
export default function InstructorDiscussionsPage() {
  ...
  const generalThreads = DISCUSSION_THREADS.filter(t => t.forum_type === "general") as InstructorDiscussionThread[];
  ...
  if (typeof view === "object" && view.type === "course") {
    ...
    const courseThreads = DISCUSSION_THREADS.filter(t => t.forum_type === "course" && t.course_id === view.courseId) as InstructorDiscussionThread[];
    return (
      <ForumList
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Breadcrumb items={[{ label: "Instructor Hub" }, { label: "Discussions Overview" }]} />
          ...
          <div className="p-6 border-b border-border bg-gradient-to-b from-background to-muted/10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground font-medium">Global student access</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground leading-relaxed">
              A public forum where all students and instructors can create, comment, and engage with discussions on any topic unbounded by a curriculum.
            </p>
          </div>
          ...
          <div className="p-6 border-b border-border bg-gradient-to-b from-background to-muted/10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground font-medium">Restricted curriculums</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Course-specific forums for verified learners. As an instructor, you can trigger new prompts here and mark them as gradable for participation points.
            </p>
          </div>
          <div className="flex-1 divide-y divide-border bg-background">
