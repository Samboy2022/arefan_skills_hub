...
import { PermissionAction, RolePermission } from "@/lib/tenant-types";
...
export default function EditRolePage({ params }: EditRolePageProps) {
  ...
  const [scope, setScope] = useState<"system" | "campus" | "department">(
    role?.scope || "system"
  );
  const [mfaRequired, setMfaRequired] = useState(role?.mfaRequired || false);
  const [ipRestricted, setIpRestricted] = useState(role?.ipRestricted || false);
  const [sessionTimeout, setSessionTimeout] = useState(role?.sessionTimeout || "1h");
  const [defaultDashboard, setDefaultDashboard] = useState(
    role?.defaultDashboard || "Admin Dashboard"
  );
  const [allowedDepartments, setAllowedDepartments] = useState<string[]>(
    role?.allowedDepartments || []
  );

  // State for permissions matrix
  const [permissions, setPermissions] = useState<Record<string, Set<PermissionAction>>>(() => {
    const initialPerms = FEATURES.reduce((acc, feature) => {
    ...
    if (role) {
      role.permissions.forEach(p => {
        if (initialPerms[p.feature]) {
          p.actions.forEach(act => initialPerms[p.feature].add(act));
        }
      });
    }
      ...
      <div className="space-y-6">
        ...
        <Card className="p-8 text-center flex flex-col items-center justify-center border-dashed">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-sm">
            The user role with ID &quot;{roleId}&quot; does not exist or has been removed from the system.
          </p>
        </Card>
      </div>
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // Simulate API call to save role changes
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <PageHeader
        ...
        description="Modify role description, operational policies, and configure fine-grained permissions."
        titleAction={
          <Button variant="outline" asChild>
            <Link href="/school-admin/roles">
              <ArrowLeft className="mr-2 h-4 w-4" />
      ...
      {role.isSystem && (
        <div className="flex gap-3 p-4 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50">
          <AlertTriangle className="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <h4 className="text-sm font-semibold text-amber-800 dark:text-amber-400">System Role Warning</h4>
            <p className="text-xs text-amber-700 dark:text-amber-500 leading-normal">
              This is a standard system role. To maintain core application integrity, the role name and description are read-only, but you can customize active security settings, dashboards, departments, and granular module permissions.
            </p>
          </div>
        </div>
      )}
