...
export default async function SchoolAdminCourseViewPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const course = await getRichCourseData(id);
  ...
  const rosterHref = `/school-admin/students?courseId=${encodeURIComponent(course.id)}`;
  const gradingEditHref = `/school-admin/courses/${course.id}/grading`;
  const publishHref = `/school-admin/courses/${course.id}/publish`;
